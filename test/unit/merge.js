...
describe('Merge', function () {
  ...
  beforeEach('instanciate merge', function () {
    ...
    // this.pouch.put must to be spied manually because of test data builders.
    // But if spied, it will be restored automatically (see below).
  })
  afterEach(function () {
    const { restore } = this.pouch.put
    if (restore) restore()
  })
  ...
  describe('moveFile', function () {
    ...
    onPlatforms('win32', 'darwin', () => {
      it('resolves an identity conflict with an existing file', async function () {
        ...
        const was = builders.file().path('baz').remoteId().upToDate().build()
